...
function readJson () {
  ...
  fs.readFile(__dirname + '/isochrone.json', function(err, data1) {
    ...
    readStationData();
    // fs.readFile(__dirname + '/station-data-with-location/lineSU.json', function(err, data2) {
...
function readStationData() {
  if (lineCount > -1) {
    ...
    fs.readFile(__dirname + '/station-data-with-location/line'+lines[lineCount]+'.json', function(err, data2) {
      ...
      writeFile(nearStationsObj);
      lineCount--;
      setTimeout(readStationData, 1000);
    });
  } else {
    console.log('done!');
  }
...
function isMarkerInsidePolygon(marker, poly) {
    ...
    var polyPoints = poly.features[3].geometry.coordinates[0];
    contour = poly.features[3].properties.contour;
    var x = marker.lat;
    var y = marker.lon;
